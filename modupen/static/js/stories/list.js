...
function getStories(num) {
  ...
  filterOption = localStorage.getItem('filter_option') || 'processing';
	if (!(filterOption.match('processing|closed'))) filterOption = 'processing';

  sortingOption = localStorage.getItem('sorting_option') || 'updated_at';
	if (!(sortingOption.match('updated_at|created_at|hits|favorites'))) sortingOption = 'updated_at';

  data['filterOption'] = filterOption;
  data['sortingOption'] = sortingOption;
  data['pagination'] = num;

  if (sortingOption == 'created_at' && num > 1) {
		data['lastStoryID'] = $('#story-list .story').last().attr('data-story-id');
	}
...
$(document).ready(function() {
	...
	$(document).on('click', '#update-story-list-option-btn', function() {
		...
		$('#update-story-list-option-btn').button('reset');
	});
});

$(window).load(function() {
	// Select value for filter and sorting option
  filterOption = localStorage.getItem('filter_option') || 'processing';
	if (!(filterOption.match('processing|closed'))) filterOption = 'processing';
  ...
  sortingOption = localStorage.getItem('sorting_option') || 'updated_at';
	...
	$('#filter-option option[value=' + filterOption + ']').attr('selected', 'selected');
	$('#sorting-option option[value=' + sortingOption + ']').attr('selected', 'selected');
